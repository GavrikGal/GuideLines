...
def create_user_guide_and_go_to_guide_page(
    ...
    :param test: FT вызывающий этот метод
    ...
    """
    user = create_user_and_pre_authenticated_session(test.browser, test.live_server_url)
    guide = create_guide(user, description=description, cover_path=cover_path)
    detail_guide_page = DetailGuidePage(test, guide.pk)
